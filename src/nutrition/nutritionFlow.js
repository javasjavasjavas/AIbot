...
//
// Reqs:
// - ../whatsapp.js -> sendText
// - ./state.js -> getState
// - ./parsers.js -> normalizeText, parse*()
// - ./geminiClient.js -> getPlanJson
// - ./prompts.js -> buildMetaPrompt, buildDayPrompt, buildShoppingPrompt
// - ./validation.js -> validateMeta, validateDayPlan
// - ./sender.js -> sendPlanDetailed
...
async function getMetaStrict(profile) {
  ...
  // Último: prompt endurecido
  const hardPrompt =
    buildMetaPrompt(profile) +
    "\n\nIMPORTANTE: Ningún valor puede ser 0. Respetá el esquema EXACTO. No agregues campos.";
...
export async function handleNutritionMessage(api, waId, text, ctx = {}) {
  ...
  if (step === "analysis") {
    ...
    // 1) META 100% IA (sin fallback hardcode)
    let meta;
    try {
      meta = await getMetaStrict(state.nutritionProfile);
    } catch (e) {
      if (LOG_LEVEL === "debug") console.error("❌ Meta error:", e?.message || e);
      await sendText(api, waId, "Hubo un problema generando tu diagnóstico/targets. Probemos de nuevo en 1 minuto.");
      return;
    }
    ...
    const pri = cleanList(meta?.prioridades_semana, 6, 200);

    // Primer bloque (100% IA)
    await sendText(
      api,
      waId,
      `🧠 *Diagnóstico breve:*\n${diag}\n\n` +
    ...
    // asegurar 7 días (si faltó alguno lo pedimos secuencialmente)
    const plan_7_dias = [];
    for (const d of days) {
      if (planByDay.has(d)) {
        ...
        try {
          ...
          if (LOG_LEVEL === "debug") console.error(`❌ Day ${d} final error:`, e?.message || e);
          await sendText(api, waId, `No pude generar el Día ${d} con valores consistentes. ¿Querés que lo rehaga?`);
          return;
        }
      }
    }
